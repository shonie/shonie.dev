// Preview-only non-module build of lib/brick-logo.js (sets window.shonieBrick).
(function(){
...
function geo() {
  const cx = 100;
  const rx = 70, ry = 35;            // top diamond half-w / half-h (2:1 dimetric)
  const topY = 80;                   // diamond vertical centre
  const d = 56;                      // body depth
  const T = [cx, topY - ry];         // back corner
  const R = [cx + rx, topY];         // right corner
  const B = [cx, topY + ry];         // front corner
  const L = [cx - rx, topY];         // left corner
  const Lp = [L[0], L[1] + d];
  const Bp = [B[0], B[1] + d];
  const Rp = [R[0], R[1] + d];
...
function onTop(g, a, b) {
  return [g.cx + g.rx * a - g.rx * b, (g.topY - g.ry) + g.ry * a + g.ry * b];
}

function studCenters(g) {
  const s0 = 0.27, s1 = 0.73;
  return [
    onTop(g, s0, s0), // back
    onTop(g, s0, s1), // left
    onTop(g, s1, s0), // right
...
const STUD_RX = 13, STUD_RY = 6.5, STUD_H = 12;
...
window.shonieBrick = shonieBrick;
})();
